...
def _targets(
    ...
    *,
    ...
    if remove_all:
        selected = _cached_commits(cache.sdk_root) | _cached_commits(cache.generated_root)
        ...
        records=tuple(
            SdkRemovalRecord(
                ...
                generated_schemas=_artifact_status(
                    target.generated_schemas, selected=True, dry_run=dry_run
                ),
...
def remove_sdk_artifacts(
    ...
    *,
    ...
    result = _result(targets, include_decoders=include_decoders, dry_run=dry_run)
    if dry_run:
        return result
    for target in targets:
        if include_decoders:
            remove_decoder(target.commit, cache=cache)
        remove_sdk(target.commit, cache=cache)
